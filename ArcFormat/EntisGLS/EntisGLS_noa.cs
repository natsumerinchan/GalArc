...
namespace ArcFormat.EntisGLS
{
    class EntisGLS_noa
    {
        ...
        public struct EntisGLS_noa_entry
        {
        ...
        public class EntisGLS_noa_timeStamp
        {
        ...
        public static int noa_unpack(string filePath)
        {
            if (Path.GetExtension(filePath) != ".noa")
                return 1;
            FileStream fs = new(filePath, FileMode.Open, FileAccess.Read);
            BinaryReader br = new(fs);
            EntisGLS_noa_header header = new();
            ...
            EntisGLS_noa_entry_header entry_header = new();
            ...
            DisplayFace.displayUn((int)entry_header.fileCount, filePath);
            List<EntisGLS_noa_timeStamp> l = new();

            long pos = 0;
            for (int i = 0; i < entry_header.fileCount; i++)
            {
                EntisGLS_noa_entry entry = new();
                ...
                entry.fileName = ArcEncoding.Encodings(932).GetString(br.ReadBytes((int)(entry.fileNameLen - 1)));
                br.ReadByte();
                pos = fs.Position;
                fs.Seek((long)entry.offset, SeekOrigin.Begin);
                br.ReadBytes(16);
                byte[] buffer = br.ReadBytes((int)entry.fileSize);
                FileStream fw = new(Path.GetDirectoryName(filePath) + "\\" + Path.GetFileNameWithoutExtension(filePath) + "\\" + entry.fileName, FileMode.Create, FileAccess.Write);
                fw.Write(buffer);
                fw.Close();
            ...
            File.WriteAllText(Path.GetDirectoryName(filePath) + "\\" + Path.GetFileNameWithoutExtension(filePath) + "\\" + "TimestampInfo.json", jsonStr);

            fs.Close();
            return 0;
        }

        public static int noa_pack(string folderPath)
        {
            string filePath = Path.GetDirectoryName(folderPath) + "\\" + Path.GetFileName(folderPath) + ".noa.new";
            ...
            //compute index size
            ulong indexSize = 4 + (ulong)Util.GetNameLenSum(filteredFiles) + (ulong)fileCount + (ulong)(40 * fileCount);
            bw.Write(indexSize);
            bw.Write(fileCount);
            ...
            foreach (var file in filteredFiles)
            {
                ...
                bw.Write(0);
                bw.Write(ArcEncoding.Encodings(932).GetBytes(Path.GetFileName(file)).Length + 1);
                bw.Write(ArcEncoding.Encodings(932).GetBytes(Path.GetFileName(file)));
                bw.Write('\0');
                i++;
            }
            foreach (var file in filteredFiles)
